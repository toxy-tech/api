$(() => {
    ...
    $(document).on('click', '#edit-api-key', function(e) {
        ...
        ToxyTech.showSuccess(window.trans.api.api_key_edit_enabled || 'API key field is now editable.');
    });
    ...
    $('#generate-api-key').on('click', function(e) {
        ...
        // Show success message
        ToxyTech.showSuccess(window.trans.api.api_key_generated || 'API key generated successfully!');
    ...
    $('#service-account-file-input').on('change', function(e) {
        ...
        if (file) {
            ...
            if (file.type !== 'application/json' && !file.name.toLowerCase().endsWith('.json')) {
                ToxyTech.showError(window.trans.api.invalid_json_file || 'Please select a valid JSON file.');
                return;
            }

            // Validate file size (max 2MB)
            if (file.size > 2 * 1024 * 1024) {
                ToxyTech.showError(window.trans.api.file_size_too_large || 'File size must be less than 2MB.');
                return;
            }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error === false) {
                    ...
                    // Show success message
                    ToxyTech.showSuccess(window.trans.api.file_uploaded_successfully || 'File uploaded successfully!');
                    ...
                    ToxyTech.showError(response.message || (window.trans.api.file_upload_error || 'Failed to upload file'));
                }
            },
            error: function(xhr) {
                ...
                ToxyTech.showError(errorMessage);
            },
            complete: function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error === false) {
                    ...
                    // Show success message
                    ToxyTech.showSuccess(window.trans.api.file_removed_successfully || 'File removed successfully!');
                    ...
                    ToxyTech.showError(response.message || (window.trans.api.file_remove_error || 'Failed to remove file'));
                }
            },
            error: function(xhr) {
    ...
    $('#send-notification-btn').on('click', function(e) {
        ...
        if (!title) {
            ToxyTech.showError(window.trans.api.please_enter_notification_title || 'Please enter a notification title.');
            $('#notification-title').focus();
            return;
        }

        if (!message) {
            ToxyTech.showError(window.trans.api.please_enter_notification_message || 'Please enter a notification message.');
